...
func (r *Room) handle(m *hub.Msg) *hub.Msg {
	...
	case "asset.open", "asset.del":
		...
		if m.Subj == "asset.open" {
			if as == nil {
				as = &gamed.AssetSubs{Asset: a}
				r.Assets[name] = as
			}
			r.SubAsset(m.From, as)
			return m.Reply(a)
		} else {
			if as != nil {
	...
	case "asset.new":
		...
		req.Dim = gamed.DefaultSize(req.Kind)
		if req.Pal == "" {
			req.Pal = "default"
		}
		req.Seq = []*pix.Seq{}
...
func (r *Room) handleSub(m *hub.Msg, a *gamed.AssetSubs) *hub.Msg {
	switch m.Subj {
...
func nameMsg(m *hub.Msg) (string, error) {
	var n nameData
	m.Unmarshal(&n)
	if !gamed.NameCheck.MatchString(n.Name) {
